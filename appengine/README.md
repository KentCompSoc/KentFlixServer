...
Google Cloud Pub/Sub for schedule checking of the univeristy system, so in theory
tasks which can trigger Google Cloud Functions.

The CRON contents:

* `/publish/daily-tick` A cron that ticks daily

* `/publish/hourly-tick` A cron that ticks every hour

* `/publish/weekly-tick` A cron that ticks every week

* `/publish/lecture-tick` A cron that ticks every 20 minutes during times when lecture may end

## Configuration

By default sample triggers (hourly, daily, and weekly) have been left in but, if you want to
...
* Sign with the society
    [Google Account](https://accounts.google.com/SignUp).

* All these have been done but if you need to create a Developers Console project:
...
The cron jobs will now "tick" along forever. As I mentioned above, we are not
limited to the `hourly-tick`, `daily-tick` and `weekly-tick` that are already within
